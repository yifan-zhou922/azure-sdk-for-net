...
#pragma warning disable SA1649 // File name should match first type name
    internal class DnsManagementClientArmOperation<T> : ArmOperation<T>
#pragma warning restore SA1649 // File name should match first type name
    {
        private readonly OperationOrResponseInternals<T> _operation;

        /// <summary> Initializes a new instance of DnsManagementClientArmOperation for mocking. </summary>
        protected DnsManagementClientArmOperation()
        {
        }

        internal DnsManagementClientArmOperation(Response<T> response)
        {
            _operation = new OperationOrResponseInternals<T>(response);
        }

        internal DnsManagementClientArmOperation(IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia)
        {
            _operation = new OperationOrResponseInternals<T>(source, clientDiagnostics, pipeline, request, response, finalStateVia, "DnsManagementClientArmOperation");
        }
