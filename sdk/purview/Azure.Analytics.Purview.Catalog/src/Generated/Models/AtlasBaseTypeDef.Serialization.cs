...
{
    [JsonConverter(typeof(AtlasBaseTypeDefConverter))]
    public partial class AtlasBaseTypeDef : IUtf8JsonSerializable
    {
        ...
        internal static AtlasBaseTypeDef DeserializeAtlasBaseTypeDef(JsonElement element)
        {
            ...
            return new AtlasBaseTypeDef(Optional.ToNullable(category), Optional.ToNullable(createTime), createdBy.Value, dateFormatter.Value, description.Value, guid.Value, name.Value, Optional.ToDictionary(options), serviceType.Value, typeVersion.Value, Optional.ToNullable(updateTime), updatedBy.Value, Optional.ToNullable(version), lastModifiedTS.Value);
        }

        internal partial class AtlasBaseTypeDefConverter : JsonConverter<AtlasBaseTypeDef>
        {
            public override void Write(Utf8JsonWriter writer, AtlasBaseTypeDef model, JsonSerializerOptions options)
            {
                writer.WriteObjectValue(model);
            }
            public override AtlasBaseTypeDef Read(ref Utf8JsonReader reader, System.Type typeToConvert, JsonSerializerOptions options)
            {
                using var document = JsonDocument.ParseValue(ref reader);
                return DeserializeAtlasBaseTypeDef(document.RootElement);
            }
        }
    }
}
